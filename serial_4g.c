...
**/
    ...
    res = pthread_create(&tpid_1[0], NULL, do_Send_WifiList_Test, (void *)&args1[0]);
    if (res) {
        perror("Thread Create error");
        exit(FALSE);
    }

    res = pthread_create(&tpid_2[1], NULL, do_Send_WifiList_Test_II, (void *)&args2[0]);
    if (res) {
        perror("Thread Create error");
        exit(FALSE);
    }
...
**/
    ...
    for(i = 0; i < 1; i++)
    {
        pthread_join(tpid_1[i], NULL); 
    }

    for(i = 0; i < 1; i++)
    {
        pthread_join(tpid_2[i], NULL); 
    }
